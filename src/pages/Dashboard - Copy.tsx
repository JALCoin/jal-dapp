...
export default function Landing({ initialPanel = "none" }: LandingProps) {
  ...
  // pick best starting panel (prefer URL, then session, then connected->grid, then initial)
  const resolveStartPanel = useCallback(
    (url: URLSearchParams, connectedNow: boolean, initial: Panel): Panel => {
      const fromUrl = url.get("panel");
      const fromSession = sessionStorage.getItem("landing:lastPanel");
      const valid = (x: any): x is Panel => isPanel(x);
      return valid(fromUrl) ? (fromUrl as Panel)
        : valid(fromSession) ? (fromSession as Panel)
        : connectedNow ? "grid"
        : initial;
    },
    []
  ...
  useEffect(() => {
    const start = resolveStartPanel(params, connected, initialPanel);
    setActivePanel(start);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // keep URL in sync, but avoid churn
  useEffect(() => {
  ...
  // expose helpers to open/close hub
  useEffect(() => {
  ...
  // if connected but hub is closed (e.g. after returning from generator), reopen
  useEffect(() => {
    if (connected && activePanel === "none") setActivePanel("grid");
  }, [connected, activePanel]);

  // persist panel aggressively on hide/unload
  useEffect(() => {
    const persist = () => sessionStorage.setItem("landing:lastPanel", activePanel);
